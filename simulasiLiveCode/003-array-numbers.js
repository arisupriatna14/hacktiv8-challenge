...
FOR i equals 0; i < length array numbers; i++
	IF i equals 0
		PUSH numbers[i] + numbers[i + 1] IN sum
	ELSE IF i equals length array numbers - 1
		PUSH numbers[i - 1] + numbers[i] IN sum
	ELSE PUSH numbers[i - 1] + numbers[i] + numbers[i + 1] IN sum
ENDFOR

FOR i equals 0; i < length array sum; i++
	IF sum[i] modulo 2 equals 1
		result = result + sum[i]
	ENDIF
ENDFOR
...
function numberCruncher(numbers) {
  ...
  for(var i=0;i<numbers.length;i++) {
    var store=0;
    if(i===0) {
      store=numbers[i]+0+numbers[i+1];
      arr.push(store);
    }else if(i===numbers.length-1) {
      store=numbers[i]+numbers[i-1]+0;
      arr.push(store);
    }else {
      store=numbers[i]+numbers[i+1]+numbers[i-1];
      arr.push(store);
    }
  }
  var output=0;
