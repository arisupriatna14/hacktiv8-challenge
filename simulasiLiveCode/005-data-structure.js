...
function getReport(studentGrades) {
  ...
  //classCode unik
  var kelas=[];
  kelas.push(grades1[0].classCode);
  for(var i=0;i<grades1.length;i++) {
    var store=1;
    for(var j=0;j<kelas.length;j++){
      if(grades1[i].classCode!==kelas[j]){
        store*=1;
      }else {
        store*=0
      }  
    ...
    if(store===1){
      kelas.push(grades1[i].classCode);
    } 
  }
  
  //dataRekap  
  for(var l=0;l<kelas.length;l++) {
    var rekap={};
    rekap.classCode=kelas[l];
    rekap.passed=[];
    rekap.failed=[];
    output.push(rekap);
  }
  
  //cari yang lulus dan gagal
  
  for(var m=0;m<output.length;m++) {
    for(var n=0;n<grades1.length;n++) {
       if(output[m].classCode===grades1[n].classCode) {
         if(grades1[n].score>=70){
           output[m].passed.push(grades1[n].name);
         }else {
           output[m].failed.push(grades1[n].name);
         }
       }
     }
    }
